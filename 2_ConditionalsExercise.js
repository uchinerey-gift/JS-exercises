...
function exercise4(varA, varB) {
  ...
  // Write your code for the exercise below here:
  // --------------------------------------------
  const aOK = typeof varA === "string" || typeof varA === "number";
  const bOK = typeof varB === "string" || typeof varB === "number";

  if (aOK && bOK && varA == varB) { // value equality allowed across string/number
...
/*
Q&A (Exercise 5):
- "5" (string) vs 5 (number): With strict equality (===) they are NOT equal; with loose (==) they are.
- NaN vs NaN: NaN is never equal to itself, so NaN == NaN is false.
- true vs 1 and false vs 0: With loose equality, true == 1 and false == 0 are true (can be surprising).
- "" (empty string) vs 0: "" == 0 is true with loose equality (surprising).
- "  " (spaces) vs 0: "  " == 0 is also true after coercion.
- [5] (array with one number) vs 5: [5] == 5 is true due to coercion to "5".
- null vs undefined: null == undefined is true (but both are different types).
- Objects (e.g., {} or new Number(5)) rarely equal primitives unless converted; {} == {} is false because different references.
Takeaway: Type coercion in loose equality (==) can make things “look equal” when they’re not strictly the same.
*/
...
function exercise8(amount1, amount2, minimum, maximum) {
  ...
  const a1In = amount1 >= low && amount1 <= high;
  const a2In = amount2 >= low && amount2 <= high;

  answer8 = a1In && a2In ? true : false;
  // --------------------------------------------
  // And above here
  // --------------------------------------------
  return answer8;
...
/*
Q&A (End Reflection):
I found this moderate: the tricky parts were equality corner cases and remembering
FizzBuzz’s 15-first rule. References I checked after trying on my own:
- MDN: if/else, switch, equality vs strict equality (===), Number.isNaN.
These helped confirm my answers.
*/
